...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const user = await User.create({
    fullName,
    avatar: avatar.url,
    coverImage: coverImage?.url || "", // setting emty incase its not given
    email,
    password,
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!email && !username) {     //incase you want to use or you  write it as if (!(email || username))
        ...
        pipeline: [
          {
            $lookup: {
              ...
              pipeline: [
                {
                  $project: {        //projection of owner field
                    fullName: 1,
                    username: 1,
                    avatar: 1
                  }
